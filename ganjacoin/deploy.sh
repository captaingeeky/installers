...
function install_prerequisites()
{
    ...
    echo -ne "${YELLOW} [###               ]\r"
    sudo apt install -y pwgen build-essential pkg-config libevent-dev libtool libboost-all-dev libgmp-dev libssl-dev libcurl4-openssl-dev libdb5.3++-dev > /dev/null 2>&1
    echo -ne " [########          ]\r"
...
function prepare_QT()
{
  ...
  echo -e " ${GREEN}MN01 ${BLUE}as an alias. Write it in the label field and click ok. Right-click on that new address and select COPY${NC}"
  echo
  ...
  echo -e " click on the ${GREEN}Send${BLUE} tab. In the Pay To field, enter the new masternode address by pasting the one you copied in "
  echo -e " the previous step, then enter the quantity ${GREEN}30,000${BLUE} and press ${GREEN}SEND${NC}."
  echo -e " ${GREEN}Enter your wallet password when prompted to confirm the transaction.${NC}"
  echo

  echo -e "${BLUE} Step 3. Now we need the ${YELLOW}Transaction ID and Output${BLUE} of the transaction you just did. For this, you simply"
  echo -e " click on ${GREEN}Help${BLUE} at the top of your wallet and click ${GREEN}Debug Window ${BLUE}. On the popup window,"
  echo -e " click on ${GREEN}Console${BLUE}. In the little bar below, type ${GREEN}masternode outputs${BLUE} and press ENTER."
  ...
  echo -e " If you can't see anything between the brackets it's because your transaction is still pending. If it doesn't work right away, try"
  echo -e " again in few minutes."
