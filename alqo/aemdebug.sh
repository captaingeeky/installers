...
function install_prerequisites()
{
  if [ "$IS_INSTALLED" = true ]; then
    ...
    sudo apt update
    echo -ne "${GREEN} >Progress: ${BLUE}[##########----]\r"
    sudo apt install -y jq libdb5.3-dev libdb5.3++-dev libdb4.8-dev libdb4.8++-dev
    if [ $? -ne 0 ]; then
       echo 
    ...
    sudo apt install -y jq libzmq5 libboost-system1.58.0 libboost-filesystem1.58.0 libboost-program-options1.58.0 libboost-thread1.58.0 libboost-chrono1.58.0 libminiupnpc10 libevent-pthreads-2.0-5 unzip
    if [ $? -ne 0 ]; then
       echo 
...
function copy_binaries()
{
  #check if version is current before copying binaries
    ...
    if [ ! -f '/usr/local/bin/aem.sh' ]; then
      wget -O /usr/local/bin/aem.sh https://raw.githubusercontent.com/zaemliss/installers/master/atheneum/aem.sh
      chmod +x /usr/local/bin/aem.sh
      echo "alias aem='/usr/local/bin/aem.sh'" >> ~/.bashrc
    fi
